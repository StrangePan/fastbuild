...
BFFVariable * BFFVariable::ConcatVarsRecurse( const AString & dstName, const BFFVariable & other, const BFFToken * operatorIter ) const
{
    ...
    if ( srcType != dstType )
    {
             ...
             ( srcType == BFFVariable::VAR_STRING ) )
        {
            ...
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, m_Token, values ) );
            return result;
        }
             ...
             ( srcType == BFFVariable::VAR_STRUCT ) )
        {
            ...
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, m_Token, values, VAR_ARRAY_OF_STRUCTS ) );
            return result;
        }
             ...
             ( ( ( srcType == BFFVariable::VAR_ARRAY_OF_STRUCTS ) || ( srcType == BFFVariable::VAR_ARRAY_OF_STRINGS ) ) && dstIsEmpty ) )
        {
            ...
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, m_Token, src->m_Type ) );
    ...
    else
    {
        ...
        {
            const unsigned int num = (unsigned int)( varSrc->GetString()->GetLength() + varDst->GetString()->GetLength() );
            SharedPtr<AString> finalValue;
            finalValue.Emplace( num );
            finalValue->Append( *varDst->GetString() );
            finalValue->Append( *varSrc->GetString() );
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, varSrc->m_Token, finalValue ) );
            return result;
        }
        ...
        if ( srcType == BFFVariable::VAR_ARRAY_OF_STRINGS )
        {
            ...
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, varSrc->m_Token, values ) );
            return result;
        }
        ...
        if ( srcType == BFFVariable::VAR_ARRAY_OF_STRUCTS )
        {
            ...
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, varSrc->m_Token, values, VAR_ARRAY_OF_STRUCTS ) );
            return result;
        }
        ...
        if ( srcType == BFFVariable::VAR_INT )
        {
            ...
            BFFVariable * result = FNEW( BFFVariable( dstNameShared, varSrc->m_Token, newVal ) );
            return result;
        }
        ...
        {
            const Array<BFFVariable> & srcMembers = *varSrc->GetStructMembers();
            const Array<BFFVariable> & dstMembers = *varDst->GetStructMembers();

            BFFVariable * const result = FNEW( BFFVariable( dstNameShared, varSrc->m_Token, BFFVariable::VAR_STRUCT ) );
